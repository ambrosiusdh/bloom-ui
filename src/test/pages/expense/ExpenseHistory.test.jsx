...
vi.mock('@api/expense.js', () => ({ getExpenses: vi.fn() }));
...
it('covers loading, error/retry, empty, and navigable creation without needing a session', async () => {
    let reject;
    getExpenses.mockImplementationOnce(() => new Promise((_, fail) => { reject = fail; }));
    ...
    getExpenses.mockResolvedValue(response([], 0));
...
it('renders returned money, classification, audit status, and session links with one paged request', async () => {
    getExpenses.mockResolvedValue(response([row, { ...row, id: 30, voided: true, voidedReason: 'Salah', voidedBy: 'manager' }], 2));
    ...
    await waitFor(() => expect(getExpenses).toHaveBeenLastCalledWith(2, 10));
...
it('canonicalizes unsupported query fields, ignores stale responses and recovers an out-of-range page', async () => {
    let resolve;
    getExpenses.mockImplementationOnce(() => new Promise(done => { resolve = done; }))
    ...
    await waitFor(() => expect(getExpenses).toHaveBeenCalledWith(1, 10));
    ...
    await waitFor(() => expect(getExpenses).toHaveBeenLastCalledWith(1, 10));
});
