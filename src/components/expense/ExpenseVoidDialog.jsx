...
const getStatusMessage = state => {
    if (state.pending) {
        return 'Memproses dan memeriksa data server...';
    }

    if (state.outcome === 'confirmed') {
        return 'Server mengonfirmasi pengeluaran sudah dibatalkan. Alasan, waktu, dan petugas di bawah adalah audit yang tersimpan, termasuk jika pembatalan dilakukan sebelumnya.';
    }

    if (state.outcome === 'storageError') {
        return 'Pemulihan tidak dapat disimpan di perangkat. Permintaan belum dikirim. Periksa ulang data untuk mencoba lagi.';
    }

    if (state.outcome === 'readError') {
        return 'Data pengeluaran gagal diverifikasi. Periksa hasil sebelum melanjutkan.';
    }

    if (state.attempt) {
        return 'Hasil belum pasti. Alasan dikunci; pemulihan hanya untuk pengeluaran yang sama.';
    }
...
export default function ExpenseVoidDialog({ onExited }) {
    ...
    useEffect(() => {
        setError('');
        if (!state.pending && state.outcome !== 'ready') {
