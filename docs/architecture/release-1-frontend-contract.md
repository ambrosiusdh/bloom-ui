...
- FE-28 single-receipt supplier payment transaction behavior is implemented: receipt detail accepts partial/full CASH, BANK_TRANSFER, or QRIS payments with confirmation, durable same-request/key recovery, duplicate prevention, conflict handling, and focused success. Only CASH requires the verified current session. Receipt amounts/status are refreshed from the backend after posting; failed refreshes cannot trigger another payment. Production approval is `BLOCKED` because persisted recovery is owned by reusable username rather than an immutable authenticated account identifier. See [transaction plan and gate evidence](../plans/fe-28-supplier-payment.md).
- FE-29 expense history and creation transaction behavior is implemented: `/expenses` renders paged backend audit records across all sessions; `/expenses/new` accepts decimal amount, one of six backend categories, and the supported description field labelled “Alasan / catatan”. New posting requires a verified open session, confirmation, and a fresh session check. The confirmed `expectedCashSessionId` is sent and persisted with the exact request/key; retries never retarget another session, and old uncertain attempts without a verified session remain locked for manual reconciliation. Production approval is `BLOCKED` because persisted recovery is owned by reusable username rather than an immutable authenticated account identifier. No edit/delete/void or drawer calculation is included; see the [contract evidence and history/create review split](../plans/fe-29-expenses.md).
- FE-30 expense void/reversal transaction behavior, eligibility handling, audit rendering, and review fixes are implemented and merged. Production approval remains `BLOCKED` on the same immutable authenticated account identifier required by FE-28 and FE-29; username-only or ownerless recovery must remain quarantined rather than reassigned by username.
...
unchanged. FE-28 and FE-29 transaction behavior is implemented, but both remain
blocked from production approval with FE-30 until recovery ownership uses an
immutable backend account identifier.
...
Recovery identity audit: FE-28, FE-29, and FE-30 transaction implementations are present, but production approval is `BLOCKED`. `/api/auth/current` exposes username/name/role only, while backend user deletion/recreation allows username reuse. A stable, non-recycled authenticated account identifier and safe legacy-recovery handling are required in each domain. FE-30's fixes invalidate history only for changed backend state, compare immutable expense facts without decimal rounding, retain confirmed voids against stale reads, and explain the one-unresolved-reversal-per-tab policy. See [evidence and review disposition](../plans/fe-30-expense-reversal.md#review-disposition-and-remaining-backend-prerequisite).
