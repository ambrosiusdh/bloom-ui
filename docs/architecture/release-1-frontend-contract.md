...
- FE-13 stock adjustment is implemented: `/stock-adjustments` and its create/detail routes use decimal editing strings, explicit STORE/WAREHOUSE and ADD/REMOVE/CORRECTION semantics, backend-reported whole/fractional item policy, one frozen confirmation, durable ambiguous-outcome quarantine, and backend-confirmed adjustment/movement results. Active items are read across every backend page instead of through a fixed inventory ceiling. Bulk CSV is not exposed in the Release 1 UI, and the browser never calculates resulting stock.
...
The current application includes the implemented Release 1 routes for login, dashboard, cashier, items, item categories, stock operations, cash sessions, sales, suppliers, goods receipts/payables/payments, and expenses. FE-19 code is present, but physical scanner verification on the store laptop is still outstanding.
...
storage keys, persisted payloads, session binding, and backend authority remain
unchanged by the 2026-09-13 identity migration. `/api/auth/login` and
`/api/auth/current` now expose `accountId`, the string form of the generated user
primary key that operations treat as a non-reusable account-lifecycle identifier. Every protected
backend request revalidates that exact ID before controller work. FE-28, FE-29, and FE-30 require
an exact match for recovery. Legacy username-only/ownerless state is retained but
quarantined for manual reconciliation; it is never adopted by matching username.

Identity completion verification (2026-09-13): 91 tests across 5 focused
auth/payment/expense workflow files passed, including duplicate/recovery/session,
same-username account recreation, legacy quarantine, and late-response coverage.
The full frontend suite passed 64 files / 384 tests; touched-file ESLint and the
production build passed. Existing Browserslist-data and large-bundle warnings
remain. No live expense or supplier payment was posted.

FE-13 review-correction verification (2026-09-13): ambiguous requests and malformed
HTTP-success responses remain durably quarantined until explicit manual-reconciliation
acknowledgement. Conflict and confirmed-success refresh failures keep stale interaction locked.
Focused FE-13 coverage passed 6 files / 30 tests; the full suite passed 64 files / 395 tests,
followed by the production build and touched-file ESLint.
...
FE-26 completed the quantity-entry reuse checkpoint. `BloomQuantityField` preserves controlled editing strings, accepts comma/dot drafts, labels UOM, and performs exact decimal-string/BigInt ±1 stepping. Receipt, FE-18, and FE-13 use it with their own validation, minimum/zero policy, commit/focus behavior, and request mapping. FE-13 CORRECTION permits zero and owns absolute-target/direction/location semantics. The legacy `BloomInputNumber` is not used for receipt or adjustment creation.
...
## 12. Known backend gates and decisions

The following gates are tracked here; completed entries document the contract that the frontend consumes:
...
- Store-laptop verification of the observed E81W scanner's physical input, focus, feedback, and rapid-scan behavior remains unresolved.
...
- The Release 1 operational dashboard read model is implemented and consumed by FE-31.

Any unresolved gate must remain named in the frontend roadmap; the frontend must not invent its contract.

## 13. Definition of done for a frontend domain PR

Recovery identity audit (2026-09-13): `/api/auth/login` and `/api/auth/current` expose the immutable user primary key as string `accountId`. Existing sessions missing that field are invalidated and must authenticate again; current-session refresh looks up the exact ID rather than username. FE-28, FE-29, and FE-30 now require that exact ID before display, edit, replay, acknowledgement, or refresh. Same-username account recreation and legacy ownerless/username-only quarantine are covered by focused tests. See [FE-30 evidence and disposition](../plans/fe-30-expense-reversal.md#review-disposition-and-identity-gate-resolution).
