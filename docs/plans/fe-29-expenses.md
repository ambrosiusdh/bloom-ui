...
Status: `BLOCKED` for production approval on immutable recovery account identity. Backend transaction gate, frontend implementation, review, and merge are present. JavaScript implementation only; no backend changes.
...
- The username owner comparison does not protect against backend deletion and recreation of the same username. Production approval requires a stable, non-recycled account identifier in `/api/auth/current`, followed by an FE-29-only recovery migration. Username-only, ownerless, or pre-session-binding attempts must remain quarantined for manual reconciliation and must never be reassigned by matching username.
