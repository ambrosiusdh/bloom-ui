# Backend handoff: unblock FE-30 recovery ownership
...
Unblock FE-30 expense reversal recovery by exposing a stable authenticated account identity. The frontend currently persists an unresolved expense ID and exact reason together with the verified username from `/api/auth/current`. Username alone is insufficient: the backend allows deleting a user and creating a different user with that same username. A recreated account must not acquire the original account's saved recovery.
...
Return a concrete frontend handoff containing the exact current-user response example and field semantics. FE-30 will then bind persisted recovery to that verified identifier. Existing username-only frontend recovery must remain locked for manual reconciliation; it must not be migrated by username matching. The frontend also needs account-recreation and recovery-rehydration regression tests before the merge blocker can be cleared.
