...
Status: `IMPLEMENTED`. Reversal/eligibility behavior, review fixes, and immutable recovery-account identity migration are present.
...
Inspected expense service/controller tests, `ExpenseMapperTest`, and `CashSessionPostgreSqlIntegrationTest` coverage for eligibility reads, stale eligibility, and both close/void lock orders. The 2026-09-13 backend run passed the domain, API, service, and web modules; boot integration tests could not initialize because Docker/Testcontainers was unavailable.
...
- A dedicated expense-void Zustand store owns pending state, reason, attempt, returned record, and refreshes. Before POST, it persists the exact expense/reason and verified immutable `accountId` in session storage; storage failure blocks sending. Uncertain attempts and confirmed results survive navigation/reload, and another account cannot display or replay them.
- Exact account-ID comparison protects the recovery boundary even when a username is deleted and recreated. One unresolved reversal or unacknowledged result occupies the whole browser tab, including after account changes. Other reversals in that tab remain disabled, with explanatory copy; no discard control is added. Username-only and ownerless legacy state remains quarantined.
...
## Review disposition and identity gate resolution

The external review identified a real identity gap. The shared backend prerequisite was completed on 2026-09-13:

- `/api/auth/login` and `/api/auth/current` return `accountId`, the string form of the generated user primary key, alongside profile fields.
- Current-session refresh resolves the exact ID. A deleted account invalidates its session, and a newly created user with the same username cannot inherit that session or recovery state.
- Legacy sessions without `accountId` are invalidated and must authenticate again.

FE-30 now binds selected records, reasons, attempts, confirmed audits, display, replay, acknowledgement, and refresh to that exact identifier. Existing username-only/ownerless recovery remains locked for manual reconciliation and is never migrated by username. Tests cover account recreation/rehydration and late-response isolation.

The [backend handoff](fe-30-recovery-identity-backend-handoff.md) is retained as historical implementation context; its prerequisite is resolved.
...
- Review follow-up focused checks passed: 5 files / 58 tests, including reversal interaction/store tests, expense history/API tests, cash-session API compatibility, and the shared error normalizer.
- Identity completion verification (2026-09-13) passed in the shared 5-file / 91-test recovery run, including same-username/different-ID recreation, legacy quarantine, valid same-ID rehydration, and late-response isolation.
- The final full frontend suite passed 64 files / 384 tests. Production build and touched-file ESLint passed; existing Browserslist-data and large-chunk warnings remain.
- No real expense was posted or reversed. Automated interaction coverage retains keyboard confirmation/cancellation, focus, pending lock, exact reason/replay, eligibility, conflict, success, and stale-refresh behavior.
